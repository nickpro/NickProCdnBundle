<?php
namespace Nickpro\CdnBundle\Filemanager;
...
use Nickpro\CdnBundle\Filemanager\Common\ObjectInterface;
...
class CdnFactory
{
    ...
    {   
        if (!$container->hasParameter('nickpro_cdn')) {
            throw new \InvalidArgumentException(sprintf("Cdn options were not set"));
        }
        
        $this->config = $container->getParameter('nickpro_cdn');
    }
